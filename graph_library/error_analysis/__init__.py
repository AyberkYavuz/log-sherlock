...
(:class:`~graph_library.models.ErrorSignature`, :class:`~graph_library.models.ErrorSummary`) live in the
shared ``graph_library.models`` package and are imported from there.

The package is laid out by concern:

    * :mod:`graph_library.error_analysis.fingerprint` — deterministic severity filtering,
      traceback collation, parameter masking and signature grouping,
    * :mod:`graph_library.error_analysis.llm_factory` — provider/mode to chat-model routing,
    * :mod:`graph_library.error_analysis.node` — the graph node that ties the two together.
...
lookup itself is the separate :mod:`graph_library.web_search` package, and the graph routes
between them.
