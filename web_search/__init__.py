...
The package is laid out by concern:
    ...
    * :mod:`web_search.client` — Tavily construction, query execution,
      relevance filtering and snippet formatting,
    * :mod:`web_search.node` — the graph node, which adds the error handling
...
``list[str]`` of prompt-ready snippets, so there is nothing for the ``models``
package to define. The schema for the *decision* that precedes it
(:class:`~models.LLMSearchDecision`) belongs to the Error Analysis Node, which
is what makes it, and lives in ``models`` with that node's other schemas.
