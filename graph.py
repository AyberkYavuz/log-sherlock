...
``graph_library/web_search/``, ``graph_library/prepare_output/``,
``graph_library/write_to_db/``) and merely registered below. Every stage of the
topology is now implemented; this module defines no stubs.
...
# Every node is implemented as a standalone feature package under
# ``graph_library/``. They are imported here and registered directly in
# ``build_graph`` — this module defines no node of its own.
...
from graph_library.write_to_db import write_to_db_node
...
class LogSherlockState(TypedDict, total=False):
    ...
    * **OUTPUT**  — the final deliverables returned to the caller.
    ...
    # The stable identifier for *this* investigation, and the primary key of
    # the ``investigations`` table. Supplied by the caller rather than
    # generated by the graph, which is what makes ``write_to_db`` idempotent:
    # re-running an investigation under the same id corrects the stored row
    # instead of accumulating a second one. Optional like every other input —
    # a run that omits it completes normally and simply is not persisted.
    ...
    # Whether ``structured_report`` reached PostgreSQL. Written solely by
    # ``write_to_db_node`` (single writer, no reducer) and set on every path,
    # including every failure path — that node degrades rather than fails, so
    # this flag is the only place in the state where the difference between a
    # stored investigation and an unstored one is machine-readable. The reason
    # for a ``False`` is in ``investigation_notes``.
    db_persisted: bool
