...
(:class:`~models.TimelineEvent`) lives in the shared ``models`` package and is
imported from there.

The package is laid out by concern:

    * :mod:`timeline.granularity` — adaptive bucket sizing and time arithmetic,
    * :mod:`timeline.buckets` — bucket construction, aggregation and rendering,
    * :mod:`timeline.milestones` — inflection-point detection,
    * :mod:`timeline.node` — the graph node that ties the three together.
